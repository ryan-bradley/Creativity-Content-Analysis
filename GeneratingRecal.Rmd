...
read_files <- function(number_of_coders) {
  data <- list()
  files <- 0
...
extract_question_columns <- function(data_list) {
  for (i in 1:length(data_list)) {
    ...
    data_list[[i]] <- data %>% mutate(Coder = i)
  }
...
merge_coder_responses <- function(data_list, no_of_administrative = 2) {
  number_of_coders <- length(data_list) 
  ...
  for (i in 2:number_of_coders) {
    if (i == 2) {
      ...
      multiple <- ids[[i]][ids[[i]] %in% multiple]
    }
  }
...
reliability_responses <- function(data_list, no_of_administrative = 2) {
  number_of_coders <- length(data_list) 
  number_of_questions <- length(colnames(data_list[[1]])) - no_of_administrative
  ...
  for (i in variable_columns) {
    ...
    temp <- data.frame(Variable = colnames(stacked)[i], "ChiSqu" = test$statistic, "DF" = test$parameter, "P-Value" = test$p.value) %>%
      mutate(Significance = as.factor(symnum(`P.Value`, na = FALSE, 
                     ...
                     symbols = c("***", "**", "*", ".", " "))))
    results <- suppressMessages(bind_rows(results, temp))
  }
...
# data_list[[2]]$Q7 <- NULL
# 
# data_list[[1]] <- data_list[[1]][1:70, ]
# data_list[[2]] <- data_list[[2]][30:100, ]
```
...
```{r}
# if (iteration == 1) {
#   variables <- list(Creativity = list("Q5", "Q6", "Q7"), Leadership = list("Q8", "Q9"))
# } else if (iteration == 2) {
#   variables <- list(Creativity = list("Q5", "Q6"), Leadership = list("Q8", "Q9"))
# }
...
```{r}
(data <- merge(data, user_demographics, by.x = "Q2", by.y = "userid"))
```
...
Based on these results, the only variable which is almost statistically signifacantly different country independance is *Social* (`Q4`) with the P Value incredibly close yet, just above 0.05
```{r}
data %>% select(country, Creativity) %>% table()
```
...
Based on these results, the only variable which does not have statistically significantly different levels of independance between departments is *Leadership*. 
```{r}
data %>% select(depart, Leadership) %>% table()
```
This table shows the ratio between leadership and non-leadership posts being statistically difference regardless of the `department`. The ratio of `social` to non-social posts are however statistically significantly different between different `department`s.

```{r}
data %>% select(depart, Social) %>% table()
```
